...
function search() {
  ...
  if (statusCode && !/^[1-5]\d\d$/.test(statusCode)) {
    ElMessage.warning('请输入 100～599 的状态码');
    return;
  }
  appliedFilter.value = {
...
<template>
  <AdminPage
    title="审计日志"
    description="检索管理员操作记录，追踪接口结果和请求摘要。"
  >
    <ElCard shadow="never">
      <div class="admin-filter">
        ...
        <ElDatePicker
          ...
          class="!w-[390px]"
        />
        <ElSelect v-model="filter.sortBy" placeholder="排序字段" class="!w-36">
          <ElOption label="发生时间" value="createdAt" /><ElOption
            label="管理员"
            value="username"
          /><ElOption label="请求方法" value="method" /><ElOption
            label="接口路径"
            value="path"
          /><ElOption label="状态码" value="statusCode" /><ElOption
            label="耗时"
            value="latency"
          />
        </ElSelect>
      ...
      <ElTable v-loading="loading" :data="rows" row-key="id">
        ...
        <ElTableColumn prop="method" label="方法" width="85">
          <template #default="{ row }">
            <AdminEnumTag :value="row.method" tone="primary" />
          </template>
        </ElTableColumn>
        <ElTableColumn
          prop="path"
    ...
    >
      <ElDescriptions v-if="detail" :column="1" border>
        ...
        <ElDescriptionsItem label="结果">
          HTTP {{ detail.statusCode }}；{{ detail.latency }}
          毫秒
        </ElDescriptionsItem>
        <ElDescriptionsItem label="时间">
          <AdminTime :value="detail.createdAt" />
        </ElDescriptionsItem>
        <ElDescriptionsItem label="来源 IP">{{ detail.ip }}</ElDescriptionsItem>
        <ElDescriptionsItem label="失败信息">
          {{ detail.errorMessage || '—' }}
        </ElDescriptionsItem>
        <ElDescriptionsItem label="请求审计摘要">
          <pre
            class="max-h-48 overflow-auto whitespace-pre-wrap break-all text-xs"
            >{{ detail.requestBody || '未采集' }}</pre>
        </ElDescriptionsItem>
        <ElDescriptionsItem label="响应审计摘要">
          <pre
            class="max-h-48 overflow-auto whitespace-pre-wrap break-all text-xs"
            >{{ detail.responseBody || '未采集' }}</pre>
        </ElDescriptionsItem>
      </ElDescriptions>
    </ElDrawer>
  </AdminPage>
