...
const canUseDataset = (dataset: string) => hasAccessByCodes([datasetPermissions[dataset] || '']);
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>异步数据导出</span><ElButton v-if="canRequest" type="primary" @click="openEditor">申请导出</ElButton></div></template>
    <ElAlert class="mb-4" type="info" show-icon :closable="false" title="可导出审计、财务异常元数据及商业化业务字段，单次最多 30 天、10,000 行。申请人与审批人必须不同；Scheduler 生成后由申请人在 24 小时内下载，过期文件会被清除。每次下载链接仅可使用一次，最长有效 60 秒。" />
