...
const selectableDatasets = computed(() =>
  DATA_EXPORT_DATASETS.filter(canUseDataset).map((dataset) => [
    dataset,
    DATA_EXPORT_DATASET_CONFIG[dataset].label,
  ] as const),
);
...
function openEditor() {
  ...
  form.range = [
    utcToAdminDateTime(Date.now() - 24 * 3600_000, timezoneStore.timezone),
...
async function save() {
  ...
  if (Date.parse(toAt) - Date.parse(fromAt) > 30 * 24 * 3600_000 || Date.parse(toAt) > Date.now()) { ElMessage.error('时间范围不得超过 30 天或包含未来时间'); return; }
  saving.value = true;
  try {
    await createDataExport({ dataset: form.dataset, fromAt, toAt, purpose: form.purpose.trim() }, crypto.randomUUID());
...
async function download(row: DataExportTask) {
  const { value } = await ElMessageBox.prompt('请输入此次下载的审计原因代码', '下载数据导出', { inputValidator: validateReasonCode });
...
<template>
  ...
  <ElDialog v-model="editorOpen" title="申请数据导出" width="600px"><ElForm label-width="100px"><ElFormItem label="数据集"><ElSelect v-model="form.dataset"><ElOption v-for="entry in selectableDatasets" :key="entry[0]" :label="entry[1]" :value="entry[0]" /></ElSelect></ElFormItem><ElFormItem label="时间范围"><ElDatePicker v-model="form.range" type="datetimerange" value-format="YYYY-MM-DD HH:mm:ss" start-placeholder="开始时间" end-placeholder="结束时间" class="!w-full" /><span class="mt-1 text-xs text-gray-500">按 {{ timezoneStore.timezone }} 解析后转为 UTC 提交</span></ElFormItem><ElFormItem label="用途"><ElInput v-model="form.purpose" type="textarea" :rows="3" maxlength="200" show-word-limit placeholder="说明业务用途、接收者和必要性" /></ElFormItem></ElForm><template #footer><ElButton @click="editorOpen = false">取消</ElButton><ElButton type="primary" :loading="saving" @click="save">提交审批</ElButton></template></ElDialog></div>
</template>
