...
const canRequest = computed(() => hasAccessByCodes(['data_export.request']));
...
const datasetText = (value: string) => value === 'audit_logs' ? '审计日志元数据' : value === 'finance_reviews' ? '财务异常元数据' : value;
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>异步数据导出</span><ElButton v-if="canRequest" type="primary" @click="openEditor">申请导出</ElButton></div></template>
    <ElAlert class="mb-4" type="info" show-icon :closable="false" title="仅提供审计日志与财务异常元数据，单次最多 30 天、10,000 行。申请人与审批人必须不同；Scheduler 生成后由申请人在 24 小时内下载，过期文件会被清除。" />
    ...
    <ElTable v-loading="loading" :data="rows" row-key="exportId"><ElTableColumn prop="exportId" label="任务 ID" width="95" /><ElTableColumn label="数据集" min-width="155"><template #default="{ row }">{{ datasetText(row.dataset) }}</template></ElTableColumn><ElTableColumn label="状态" width="105"><template #default="{ row }">{{ statusText(row.status) }}</template></ElTableColumn><ElTableColumn prop="requestedBy" label="申请人" width="95" /><ElTableColumn prop="reviewedBy" label="审批人" width="95" /><ElTableColumn prop="rowCount" label="行数" width="85" /><ElTableColumn prop="expiresAt" label="下载截止" min-width="175" /><ElTableColumn label="操作" width="210" fixed="right"><template #default="{ row }"><ElButton link type="primary" @click="openDetail(row)">详情</ElButton><ElButton v-if="canApprove && row.status === 'pending' && row.requestedBy !== currentUserId" link type="success" @click="review(row, true)">批准</ElButton><ElButton v-if="canApprove && row.status === 'pending' && row.requestedBy !== currentUserId" link type="warning" @click="review(row, false)">驳回</ElButton><ElButton v-if="canDownload && row.status === 'ready' && row.requestedBy === currentUserId && row.expiresAt && new Date(row.expiresAt).getTime() > Date.now()" link type="primary" @click="download(row)">下载</ElButton></template></ElTableColumn></ElTable><div class="mt-4 flex justify-end gap-2"><ElButton :disabled="cursorStack.length === 0" @click="previous">上一页</ElButton><ElButton :disabled="!nextCursor" @click="next">下一页</ElButton></div>
