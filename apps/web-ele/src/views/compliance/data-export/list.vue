...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>异步数据导出</span><ElButton v-if="canRequest" type="primary" @click="openEditor">申请导出</ElButton></div></template>
    <ElAlert class="mb-4" type="info" show-icon :closable="false" title="可按权限导出审计、用户、商业化、AI 和安全治理数据。会话与消息仅含本地元数据，审核证据只含脱敏文本与媒体 ID。用户完整邮箱、完整资料、漂流瓶内容、举报证据和申诉说明需要各自的敏感权限。单次最多 30 天、10,000 行；申请人与审批人必须不同。生成后 24 小时过期，下载链接仅可使用一次、最长有效 60 秒。" />
