...
const datasetLabels: Record<string, string> = {
  ...
  users: 'App 用户资料', user_emails: '用户完整邮箱（敏感）',
  user_profiles: '用户完整资料（敏感）',
  user_devices: '用户设备元数据（敏感）',
  appeals: '申诉说明与证据引用（敏感）', penalties: '用户处罚记录',
  conversations: '会话元数据', conversation_members: '会话成员与匿名身份',
  messages: '消息元数据',
  content_reviews: '审核记录', review_evidence: '脱敏审核证据',
  ai_roles: 'AI 角色配置', ai_generation_batches: 'AI 生成批次',
  ai_generation_items: 'AI 生成项', ai_reply_tasks: 'AI 回复任务',
  contact_requests: '官网联系请求（敏感）', contact_replies: '官网联系回复（敏感）',
  website_content_versions: '官网页面与帮助文章版本',
  legal_document_versions: '法律文档版本与多语言正文',
  config_versions: '配置版本与审批记录',
};
const datasetPermissions: Record<string, string[]> = {
  ...
  users: ['account_user.read'],
  user_emails: ['account_user.read', 'account_user.email.export_sensitive'],
  user_profiles: ['account_user.read', 'account_user.profile.export_sensitive'],
  user_devices: ['account_user.read', 'account_user.device.export_sensitive'],
  appeals: ['appeal.read', 'appeal.evidence.read_sensitive'],
  penalties: ['penalty.read'],
  conversations: ['message.read_context'],
  conversation_members: ['message.read_context'],
  messages: ['message.read_context'],
  content_reviews: ['moderation.review'],
  review_evidence: ['moderation.review'],
  ai_roles: ['ai_role.manage'],
  ai_generation_batches: ['ai_content.publish'],
  ai_generation_items: ['ai_content.publish'],
  ai_reply_tasks: ['ai_content.publish'],
  contact_requests: ['contact_request.read', 'contact_request.export_sensitive'],
  contact_replies: ['contact_request.read', 'contact_request.export_sensitive'],
  website_content_versions: ['website_content.read'],
  legal_document_versions: ['legal_document.read'],
  config_versions: ['config_version.read'],
};
...
const failureLabels: Record<string, string> = { ROW_LIMIT_EXCEEDED: '超过 10,000 行上限', GENERATION_FAILED: '生成出错，请联系管理员重新申请' };
const failureText = (value?: null | string) => failureLabels[value || ''] || value || '—';
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>异步数据导出</span><ElButton v-if="canRequest" type="primary" @click="openEditor">申请导出</ElButton></div></template>
    <ElAlert class="mb-4" type="info" show-icon :closable="false" title="可按权限导出审计、用户、商业化、AI、安全治理、配置版本、法律文档、官网内容与联系数据。会话与消息仅含本地元数据，审核证据只含脱敏文本与媒体 ID。用户完整邮箱、完整资料、设备元数据、漂流瓶内容、举报证据、申诉说明和联系往来正文需要各自的敏感权限。设备导出不含安装标识哈希、IP 哈希和 Push Token。单次最多 30 天、10,000 行；申请人与审批人必须不同。生成后 24 小时过期，下载链接仅可使用一次、最长有效 60 秒。" />
