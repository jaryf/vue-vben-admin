...
const form = reactive<{
  ...
  version: '',
  effectiveAt: '',
  translations: [{ languageCode: 'en', title: '', bodyMarkdown: '' }],
});
const types: Array<[string, string]> = [
  ...
  ['content_policy', '内容政策'],
  ['account_deletion', '账户注销说明'],
];
const typeText = (value: string) =>
  types.find(([key]) => key === value)?.[1] || value;
...
async function load(cursor = '') {
  loading.value = true;
  try {
    const result = await listLegalVersions({
...
async function openDetail(row: LegalVersion) {
  detail.value = await getLegalVersion(row.versionId);
...
async function openEditor(row?: LegalVersion) {
  ...
  Object.assign(form, {
    documentType: current?.version.documentType ?? 'terms',
    version: current?.version.version ?? '',
    effectiveAt: current
      ? utcToAdminDateTime(current.version.effectiveAt, timezoneStore.timezone)
      : '',
    translations: current
      ? current.translations.map(({ languageCode, title, bodyMarkdown }) => ({
          languageCode,
          title,
          bodyMarkdown,
        }))
      : [{ languageCode: 'en', title: '', bodyMarkdown: '' }],
  });
  editorOpen.value = true;
}
function addTranslation() {
  form.translations.push({ languageCode: '', title: '', bodyMarkdown: '' });
}
async function save() {
  const translations = form.translations.map((item) => ({
  ...
  if (
    ...
    translations.some(
      (item) => !item.languageCode || !item.title || !item.bodyMarkdown,
    ) ||
    new Set(translations.map((item) => item.languageCode)).size !==
      translations.length
  ) {
    ElMessage.error('请填写版本、生效时间及不重复的完整翻译');
    return;
  }
  ...
  try {
    ...
    await (editingId.value === null
      ? createLegalVersion({ ...payload, documentType: form.documentType })
      : updateLegalVersion(editingId.value, payload));
    editorOpen.value = false;
    ElMessage.success('法律文档草稿已保存');
...
async function action(
  ...
  const label = { review: '复核翻译', publish: '发布', retire: '退役' }[value];
  if (
    !(await confirmDialog(
      `确定${label} ${typeText(row.documentType)} ${row.version} 吗？`,
      '确认文档操作',
      { type: 'warning' },
    ))
  )
...
<template>
  <AdminPage
    title="法律文档"
    description="维护文档版本与多语言内容，按流程复核和发布。"
  >
    <template #actions>
      ...
      <ElTable v-loading="loading" :data="rows" row-key="versionId">
        <ElTableColumn
          ...
          label="版本 ID"
          width="100"
        /><!-- @vue-generic {LegalVersion} --><ElTableColumn
          label="文档类型"
          min-width="145"
        >
    ...
    >
      <template v-if="detail">
        ...
        />
        <div
          v-for="translation in detail.translations"
    ...
    <ElDrawer
      v-model="editorOpen"
      :title="
        editingId === null ? '创建法律文档草稿' : `编辑草稿 #${editingId}`
      "
      size="70%"
    >
      <ElForm label-width="110px">
        <ElFormItem label="文档类型">
          <ElSelect v-model="form.documentType" :disabled="editingId !== null">
            <ElOption
              v-for="[value, label] in types"
              :key="value"
              :label="label"
        ...
        >
          <div class="mb-3 flex justify-between">
            <strong>翻译 {{ index + 1 }}</strong><ElButton
              ...
              link
              type="danger"
              @click="form.translations.splice(index, 1)"
            >
              移除
            </ElButton>
          </div>
