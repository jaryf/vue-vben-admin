...
function publishBlockedReason(row: LegalVersion) {
  if (!row.reviewedBy) return '需先由其他管理员完成复核';
  if (!currentUserId.value) return '无法确认当前管理员身份';
  if (row.reviewedBy === currentUserId.value) return '复核人不能发布同一版本';
  return '';
}
...
async function openEditor(row?: LegalVersion) {
  ...
  Object.assign(form, {
    ...
    effectiveAt: current
      ? utcToAdminDateTime(current.version.effectiveAt, timezoneStore.timezone)
      : '',
...
async function save() {
  ...
  try {
    effectiveAt = adminDateTimeToUtc(form.effectiveAt, timezoneStore.timezone);
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '生效时间无效');
    return;
  }
  if (!effectiveAt) { ElMessage.error('请填写生效时间'); return; }
  saving.value = true;
  try {
    const payload = { version: form.version.trim(), effectiveAt, translations };
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>法律文档版本</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    ...
    <ElTable v-loading="loading" :data="rows" row-key="versionId"><ElTableColumn prop="versionId" label="版本 ID" width="100" /><ElTableColumn label="文档类型" min-width="145"><template #default="{ row }">{{ typeText(row.documentType) }}</template></ElTableColumn><ElTableColumn prop="version" label="版本" min-width="125" /><ElTableColumn label="状态" width="95"><template #default="{ row }">{{ statusText(row.status) }}</template></ElTableColumn><ElTableColumn label="翻译语言" min-width="145"><template #default="{ row }">{{ row.translationLanguages.join('、') }}</template></ElTableColumn><ElTableColumn prop="reviewedBy" label="复核管理员" width="115" /><ElTableColumn prop="effectiveAt" label="生效时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.effectiveAt" /></template></ElTableColumn><ElTableColumn label="操作" min-width="240" fixed="right"><template #default="{ row }"><ElButton link type="primary" @click="openDetail(row)">详情</ElButton><ElButton v-if="canWrite && row.status === 'draft'" link type="primary" @click="openEditor(row)">编辑</ElButton><ElButton v-if="canReview && row.status === 'draft'" link type="success" @click="action(row, 'review')">复核</ElButton><ElTooltip v-if="canPublish && row.status === 'draft'" :disabled="!publishBlockedReason(row)" :content="publishBlockedReason(row)" placement="top"><span><ElButton link type="success" :disabled="!!publishBlockedReason(row)" @click="action(row, 'publish')">发布</ElButton></span></ElTooltip><ElButton v-if="canPublish && row.status === 'published'" link type="warning" @click="action(row, 'retire')">退役</ElButton></template></ElTableColumn></ElTable>
  ...
  <ElDrawer v-model="detailOpen" title="法律文档详情" size="68%" @closed="detail = null"><template v-if="detail"><ElDescriptions :column="2" border><ElDescriptionsItem label="文档类型">{{ typeText(detail.version.documentType) }}</ElDescriptionsItem><ElDescriptionsItem label="版本">{{ detail.version.version }}</ElDescriptionsItem><ElDescriptionsItem label="状态">{{ statusText(detail.version.status) }}</ElDescriptionsItem><ElDescriptionsItem label="生效时间"><AdminTime :value="detail.version.effectiveAt" /></ElDescriptionsItem></ElDescriptions><ElAlert class="mt-4" title="安全 Markdown 预览已禁用原始 HTML；HTML 标签只会作为文本显示。" type="info" show-icon :closable="false" /><div v-for="translation in detail.translations" :key="translation.languageCode" class="mt-5"><ElDivider>{{ translation.languageCode }} · {{ translation.title }}</ElDivider><SafeMarkdownPreview :source="translation.bodyMarkdown" /></div></template></ElDrawer>
  <ElDrawer v-model="editorOpen" :title="editingId === null ? '创建法律文档草稿' : `编辑草稿 #${editingId}`" size="70%"><ElForm label-width="110px"><ElFormItem label="文档类型"><ElSelect v-model="form.documentType" :disabled="editingId !== null"><ElOption v-for="[value, label] in types" :key="value" :label="label" :value="value" /></ElSelect></ElFormItem><ElFormItem label="版本"><ElInput v-model="form.version" maxlength="64" /></ElFormItem><ElFormItem label="生效时间"><ElDatePicker v-model="form.effectiveAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" /><span class="ml-3 text-xs text-gray-500">{{ timezoneStore.timezone }}</span></ElFormItem><ElDivider>翻译内容</ElDivider><div v-for="(translation, index) in form.translations" :key="index" class="mb-5 rounded border p-4"><div class="mb-3 flex justify-between"><strong>翻译 {{ index + 1 }}</strong><ElButton v-if="form.translations.length > 1" link type="danger" @click="form.translations.splice(index, 1)">移除</ElButton></div><ElFormItem label="语言代码"><ElInput v-model="translation.languageCode" class="!w-40" /></ElFormItem><ElFormItem label="标题"><ElInput v-model="translation.title" /></ElFormItem><ElFormItem label="Markdown 正文"><ElInput v-model="translation.bodyMarkdown" type="textarea" :rows="10" /></ElFormItem><ElDivider>安全预览</ElDivider><SafeMarkdownPreview :source="translation.bodyMarkdown" /></div><ElButton @click="addTranslation">添加翻译</ElButton></ElForm><template #footer><ElButton @click="editorOpen = false">取消</ElButton><ElButton type="primary" :loading="saving" @click="save">保存草稿</ElButton></template></ElDrawer></div>
</template>
