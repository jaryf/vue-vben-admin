...
import {
  getMFAStatusApi,
  listCurrentAdminSecurityEventsApi,
...
const eventLabels: Record<string, string> = {
  login: '登录',
  login_mfa_required: '登录待双因素验证',
  logout: '退出登录',
  mfa_setup: '开始绑定双因素',
  mfa_enable: '启用双因素',
  mfa_disable: '停用双因素',
  recovery_codes_rotate: '重置恢复码',
...
async function regenerate() {
  ...
  busy.value = true;
  try {
    const result = await regenerateRecoveryCodesApi(
...
<template>
  <div class="max-w-4xl">
    ...
    <ElAlert
      ...
      :title="`双因素认证已启用；剩余 ${remaining} 个恢复码`"
      type="success"
      show-icon
      :closable="false"
    />
    <ElButton
      ...
      class="mt-5"
      type="primary"
      @click="router.push('/auth/mfa-setup')"
    >
      立即绑定
    </ElButton>
    <template v-if="enabled === true">
      ...
      <div
        v-if="recoveryCodes.length"
        class="mt-3 grid grid-cols-2 gap-2 font-mono"
      >
    ...
    <ElAlert
      v-if="enabled"
      title="展示与当前管理员相关的登录、密码及双因素操作；不保存或展示密码、验证码、恢复码和令牌。"
      type="info"
      :closable="false"
      class="mb-4"
    />
    <ElTable
      ...
      row-key="eventId"
    >
      <ElTableColumn label="事件" min-width="170">
        ...
        <template #default="{ row }">
          <AdminEnumTag
            ...
            :label="
              row.success
                ? '成功'
                : row.eventType === 'login_mfa_required'
                  ? '待验证'
                  : '未完成'
            "
      ...
      <ElTableColumn prop="occurredAt" label="发生时间" min-width="190">
        <template #default="{ row }">
          <AdminTime :value="row.occurredAt" />
        </template>
      </ElTableColumn>
    </ElTable>
    <div v-if="enabled" class="admin-pagination">
      <ElButton :disabled="cursorStack.length === 0" @click="previousEvents">
        上一页
      </ElButton>
      <ElButton :disabled="!nextCursor" @click="nextEvents">下一页</ElButton>
    </div>
  </div>
</template>
