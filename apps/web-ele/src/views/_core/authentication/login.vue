...
async function submit() {
  ...
  if (
    ...
    !(useRecoveryCode.value ? form.recoveryCode : form.totpCode)
  ) {
    errorText.value = useRecoveryCode.value
      ? '请输入恢复码'
      : '请输入六位动态验证码';
    return;
  }
  try {
    const secondFactor = useRecoveryCode.value
      ? { recoveryCode: form.recoveryCode.trim() }
      : { totpCode: form.totpCode.trim() };
    const result = await authStore.authLogin({
      username: form.username.trim(),
      password: form.password,
      ...(secondFactorRequired.value ? secondFactor : {}),
    });
    ...
    errorText.value = secondFactorRequired.value
      ? '验证失败，请检查动态验证码或恢复码'
      : '登录失败，请检查用户名和密码';
  }
}
...
<template>
  <div class="driftly-auth-card mx-auto w-full max-w-md">
    <div class="mb-8">
      <div class="driftly-auth-brand">
        <span aria-hidden="true">D</span> DRIFTLY · ADMIN
      </div>
    ...
    <form @submit.prevent="submit">
      <template v-if="!secondFactorRequired">
        <label class="mb-1 block text-sm" for="admin-username">
          管理员用户名
        </label>
        <ElInput
      ...
      <template v-else>
        ...
        <label class="mb-1 block text-sm" for="admin-second-factor">{{
          useRecoveryCode ? '一次性恢复码' : '六位动态验证码'
        }}</label>
        <ElInput
