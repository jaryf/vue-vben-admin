...
async function openEditor(row?: AIRole) {
  ...
  Object.assign(form, {
    name: value?.name ?? '',
    avatarMediaId: value?.avatarMediaId ? String(value.avatarMediaId) : '',
...
async function save() {
  ...
  const avatarMediaId = form.avatarMediaId.trim()
    ? Number(form.avatarMediaId.trim())
    : null;
  if (
    avatarMediaId !== null &&
    (!Number.isSafeInteger(avatarMediaId) || avatarMediaId < 1)
  ) {
    ElMessage.error('请输入有效头像媒体资产 ID');
    return;
  }
  if (
    !form.name.trim() ||
    !form.persona.trim() ||
    commas(form.languages).length === 0 ||
    ids.some((id) => !Number.isInteger(id) || id < 1)
  ) {
    ElMessage.error('请填写名称、人设、语言及有效兴趣标签 ID');
    return;
  }
  if (form.replyDelayMaxSeconds < form.replyDelayMinSeconds) {
    ElMessage.error('最大回复延迟不能小于最小值');
    return;
  }
  const data = {
    name: form.name.trim(),
    ...
    styleRules: lines(form.styleRules),
    safetyRules: lines(form.safetyRules),
    languages: commas(form.languages),
    interestTagIds: ids,
    allowedCategories: commas(form.allowedCategories),
    adultEnabled: form.adultEnabled,
  ...
  try {
    await (editingId.value === null
      ? createAIRole(data, createKey.value)
      : updateAIRole(editingId.value, data));
...
async function toggle(row: AIRole) {
  ...
  if (
    !(await confirmDialog(
      `确定${enable ? '启用' : '停用'} AI 角色「${row.name}」吗？`,
      '确认状态变更',
      { type: 'warning' },
    ))
  )
...
<template>
  <AdminPage title="AI 角色" description="配置角色人设、语言能力与回复规则。">
    <template #actions>
      <ElButton type="primary" @click="openEditor()"> 新增角色 </ElButton>
    </template>
    <ElCard shadow="never">
      <div class="admin-filter">
        <ElSelect
          ...
          class="!w-40"
        >
          <ElOption label="启用" value="enabled" /><ElOption
            label="停用"
            value="disabled"
          />
      ...
      <ElTable v-loading="loading" :data="rows" row-key="roleId">
        <ElTableColumn prop="roleId" label="角色 ID" width="95" /><ElTableColumn
          ...
          label="名称"
          min-width="135"
        /><!-- @vue-generic {AIRole} --><ElTableColumn
          label="头像资产 ID"
          width="120"
        >
    ...
    <ElDialog
      v-model="editorOpen"
      :title="editingId === null ? '新增 AI 角色' : '编辑 AI 角色'"
      width="650px"
      destroy-on-close
    >
      <ElForm label-width="120px" @submit.prevent="save">
        <ElFormItem label="名称"><ElInput v-model="form.name" /></ElFormItem>
        <ElFormItem label="头像资产 ID">
          <ElInput
            v-model="form.avatarMediaId"
            placeholder="已完成审核的媒体资产 ID；留空则不绑定"
            clearable
          />
        </ElFormItem>
        <ElFormItem label="人设">
          <ElInput v-model="form.persona" type="textarea" :rows="3" />
        </ElFormItem>
        <ElFormItem label="风格规则">
          <ElInput
            v-model="form.styleRules"
            type="textarea"
            :rows="3"
            placeholder="每行一条"
          />
        </ElFormItem>
        <ElFormItem label="安全规则">
          <ElInput
            v-model="form.safetyRules"
            type="textarea"
            :rows="3"
            placeholder="每行一条"
          />
        ...
        <ElFormItem label="语言代码">
          <ElInput
            ...
            placeholder="逗号分隔，例如 en, hi"
          />
        </ElFormItem>
        <ElFormItem label="兴趣标签 ID">
          <ElInput v-model="form.interestTagIds" placeholder="逗号分隔" />
        </ElFormItem>
        <ElFormItem label="允许分类">
          <ElInput
            v-model="form.allowedCategories"
            placeholder="逗号分隔分类代码"
          />
        </ElFormItem>
        <ElFormItem label="成人内容">
          <ElSwitch v-model="form.adultEnabled" />
        </ElFormItem>
        <ElFormItem label="每日回复上限">
          <ElInputNumber v-model="form.dailyMessageLimit" :min="1" />
        </ElFormItem>
        <ElFormItem label="最小回复延迟">
          <ElInputNumber v-model="form.replyDelayMinSeconds" :min="0" />
          秒
        </ElFormItem>
        <ElFormItem label="最大回复延迟">
          <ElInputNumber v-model="form.replyDelayMaxSeconds" :min="0" />
          秒
        </ElFormItem>
      </ElForm>
      <template #footer>
