  ...
  <div class="p-5">
    <ElCard shadow="never">
      ...
      <ElTable v-if="active === 'batches'" v-loading="loading" :data="rows" row-key="batchId">
        <ElTableColumn prop="batchId" label="批次 ID" width="95" /><ElTableColumn prop="categoryId" label="分类 ID" width="95" /><ElTableColumn prop="languageCode" label="语言" width="90" /><ElTableColumn prop="targetCount" label="目标数量" width="100" /><ElTableColumn prop="modelConfigVersion" label="模型配置版本" min-width="160" /><ElTableColumn label="状态" width="105"><template #default="{ row }">{{ statusText(row.status) }}</template></ElTableColumn><ElTableColumn prop="createdAt" label="创建时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn><ElTableColumn label="操作" width="220" fixed="right"><template #default="{ row }"><ElButton link type="primary" @click="openDetail(row)">详情</ElButton><ElButton link type="primary" @click="viewBatchItems(row.batchId)">内容</ElButton><ElButton v-if="row.status === 'draft'" link type="success" @click="batchAction(row, 'start')">启动</ElButton><ElButton v-if="['draft', 'queued', 'running'].includes(row.status)" link type="warning" @click="batchAction(row, 'cancel')">取消</ElButton></template></ElTableColumn>
      </ElTable>
      ...
      <ElTable v-else v-loading="loading" :data="rows" row-key="taskId">
        <ElTableColumn prop="taskId" label="任务 ID" width="95" /><ElTableColumn prop="conversationId" label="会话 ID" width="100" /><ElTableColumn prop="aiRoleId" label="角色 ID" width="100" /><ElTableColumn label="状态" width="110"><template #default="{ row }">{{ statusText(row.status) }}</template></ElTableColumn><ElTableColumn prop="attempts" label="尝试次数" width="95" /><ElTableColumn prop="lastErrorCode" label="错误码" min-width="155" /><ElTableColumn prop="scheduledAt" label="计划时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.scheduledAt" /></template></ElTableColumn><ElTableColumn label="操作" width="170" fixed="right"><template #default="{ row }"><ElButton link type="primary" @click="openDetail(row)">详情</ElButton><ElButton v-if="row.status === 'failed'" link type="success" @click="replyAction(row, 'retry')">重试</ElButton><ElButton v-if="!['sent', 'cancelled'].includes(row.status)" link type="warning" @click="replyAction(row, 'cancel')">取消</ElButton></template></ElTableColumn>
      </ElTable>
