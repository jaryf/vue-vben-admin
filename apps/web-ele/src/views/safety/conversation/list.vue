...
import {
  getConversation, getMessageContext, listConversations, listMessages, removeMessage,
...
<template>
  ...
  <ElDrawer v-model="contextOpen" title="消息上下文" size="55%" @closed="messageContext = null"><template v-if="messageContext"><ElAlert title="仅展示消息元数据，正文尚不可用。" type="warning" :closable="false" class="mb-4" /><ElDivider>前文</ElDivider><ElTable :data="messageContext.before"><ElTableColumn prop="sequenceNo" label="序号" width="80" /><ElTableColumn prop="messageId" label="消息 ID" /><ElTableColumn prop="messageType" label="类型" width="100" /></ElTable><ElDivider>目标消息</ElDivider><ElDescriptions :column="1" border><ElDescriptionsItem label="消息 ID">{{ messageContext.targetMessage.messageId }}</ElDescriptionsItem><ElDescriptionsItem label="发送者">{{ messageContext.targetMessage.senderMemberType }} #{{ messageContext.targetMessage.senderMemberId }}</ElDescriptionsItem><ElDescriptionsItem label="状态">{{ messageContext.targetMessage.status }}</ElDescriptionsItem><ElDescriptionsItem label="审核状态">{{ messageContext.targetMessage.moderationStatus }}</ElDescriptionsItem></ElDescriptions><ElDivider>后文</ElDivider><ElTable :data="messageContext.after"><ElTableColumn prop="sequenceNo" label="序号" width="80" /><ElTableColumn prop="messageId" label="消息 ID" /><ElTableColumn prop="messageType" label="类型" width="100" /></ElTable></template></ElDrawer></div>
</template>
