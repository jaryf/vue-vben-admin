...
const canExtended = computed(
  () =>
    hasAccessByCodes(['message.read_context']) &&
    hasAccessByCodes(['message.read_context_extended']),
);
const canPreview = computed(
  () =>
    hasAccessByCodes(['message.read_context']) &&
    hasAccessByCodes(['message.media.preview']),
);
...
const messageNextCursor = ref<null | string>(null);
...
const searchedNextCursor = ref<null | string>(null);
...
const messageStatusText = (value: string) =>
  messageStatuses.find(([code]) => code === value)?.[1] || value;
const moderationStatusText = (value: string) =>
  moderationStatuses.find(([code]) => code === value)?.[1] || value;
async function reason(prompt: string): Promise<null | string> {
  const result = await promptDialog(prompt, '审计原因', {
...
async function load(cursor = '') {
  ...
  try {
    const result = await listConversations({
      ...
      memberUserId: filter.memberUserId || undefined,
      type: filter.type || undefined,
      status: filter.status || undefined,
      sourceBottleId: filter.sourceBottleId || undefined,
...
async function openDetail(row: ConversationRow) {
  const reasonCode = await reason(
    `查看会话 #${row.conversationId} 的参与者和消息元数据，请填写原因`,
  );
...
async function loadMessages(cursor = '') {
  ...
  const result = await listMessages({
    conversationId: detail.value.conversation.conversationId,
    reasonCode: messageReason.value,
    cursor: cursor || undefined,
    limit: 20,
  });
  messages.value = cursor
...
async function openMessages() {
  ...
  const reasonCode = await reason(
    `浏览会话 #${detail.value.conversation.conversationId} 的消息元数据，请填写原因`,
  );
  if (reasonCode === null) return;
  messageReason.value = reasonCode;
  await loadMessages();
}
async function loadSearchedMessages(cursor = '') {
  ...
  try {
    const result = await listMessages({
      ...searchedQuery.value,
      reasonCode: searchedReason.value,
      cursor: cursor || undefined,
      limit: 20,
    });
...
async function searchAllMessages() {
  if (
    messageFilter.messageId.trim() &&
    !/^[0-9a-fA-F]{8}(-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}$/.test(
      messageFilter.messageId.trim(),
    )
  ) {
    ElMessage.warning('请输入有效的消息 UUID');
    return;
  }
  if (
    [messageFilter.conversationId, messageFilter.senderMemberId].some(
      (value) => value.trim() && !/^[1-9]\d*$/.test(value.trim()),
    )
  ) {
    ElMessage.warning('会话和发送者 ID 必须为正整数');
    return;
  }
  ...
  const reasonCode = await reason('跨会话检索消息元数据，请填写审计原因');
  if (reasonCode === null) return;
  searchedReason.value = reasonCode;
  searchedQuery.value = {
...
async function expandContext() {
  ...
  const reasonCode = await reason(
    `扩大查看消息 ${messageId} 前后各 50 条元数据，请填写审计原因`,
  );
  if (reasonCode === null) return;
...
const previewable = (row: MessageRow) =>
  canPreview.value &&
  !!row.mediaAssetId &&
  ['image', 'voice'].includes(row.messageType);
...
async function openPreview(row: MessageRow) {
  ...
  const reasonCode = await reason(
    `预览消息 ${row.messageId} 的媒体，请填写审计原因`,
  );
  if (reasonCode === null) return;
...
async function remove(row: MessageRow) {
  ...
  if (reasonCode === null) return;
  if (
    !(await confirmDialog('确定下架这条消息吗？', '确认下架', {
      type: 'warning',
    }))
  )
    return;
  ...
  if (searchedReason.value)
    await loadSearchedMessages(searchedCursorStack.value.at(-1) || '');
  if (messageContext.value?.targetMessage.messageId === row.messageId)
...
<template>
  <AdminPage
    title="会话与消息"
    description="查看会话成员与消息元数据，按权限开展审计和内容治理。"
  >
    <ElCard shadow="never">
      <ElAlert
        title="当前服务端提供会话、参与者和消息元数据，以及受控图片与语音预览；不返回消息正文。IM 正文能力尚未接通。敏感查看需填写原因并留审计。"
        type="info"
        show-icon
        :closable="false"
        class="mb-4"
      ...
      <div class="admin-filter">
        ...
        /><ElInput
          ...
          class="!w-36"
        /><ElInput
          v-model="filter.sourceBottleId"
          placeholder="来源漂流瓶 ID"
          clearable
          class="!w-40"
        /><ElInput
          v-model="filter.type"
      ...
      <ElTable v-loading="loading" :data="rows" row-key="conversationId">
        <ElTableColumn
          prop="conversationId"
          label="会话 ID"
          width="110"
        ...
        /><ElTableColumn
          ...
          label="举报数"
          width="90"
        /><ElTableColumn prop="lastMessageAt" label="最近消息" min-width="175">
          <template #default="{ row }">
    ...
    <ElCard class="mt-5" shadow="never">
      ...
      <ElAlert
        ...
        show-icon
        :closable="false"
        title="检索前需填写审计原因；每页读取均记录敏感访问。结果只包含本地消息元数据；有权限者可另行填写原因预览图片或语音。IM 正文不可用。"
      />
      <div class="admin-filter">
        <ElInput
          v-model="messageFilter.messageId"
          placeholder="消息 UUID"
          clearable
          class="!w-64"
        />
        <ElInput
          v-model="messageFilter.conversationId"
          placeholder="会话 ID"
          clearable
          class="!w-32"
        />
        <ElSelect
          v-model="messageFilter.senderMemberType"
          clearable
          placeholder="发送者类型"
          class="!w-36"
        >
          <ElOption label="App 用户" value="user" /><ElOption
            label="AI 角色"
            value="ai_role"
          />
        </ElSelect>
        <ElInput
          v-model="messageFilter.senderMemberId"
          placeholder="发送者 ID"
          clearable
          class="!w-32"
        />
        <ElSelect
          v-model="messageFilter.messageType"
          clearable
          placeholder="消息类型"
          class="!w-36"
        >
          <ElOption label="文字" value="text" /><ElOption
            label="图片"
            value="image"
          /><ElOption label="语音" value="voice" /><ElOption
            label="视频"
            value="video"
        ...
        >
          <ElOption
            v-for="[value, label] in messageStatuses"
            :key="value"
            :value="value"
            :label="label"
          />
        ...
        <ElSelect
          v-model="messageFilter.moderationStatus"
          clearable
          placeholder="审核状态"
          class="!w-36"
        >
          <ElOption
            v-for="[value, label] in moderationStatuses"
            :key="value"
            :value="value"
            :label="label"
          />
        ...
        <ElDatePicker
          v-model="messageFilter.createdRange"
          type="datetimerange"
        ...
        <ElButton
          ...
          @click="searchAllMessages"
        >
          填写原因并查询
        </ElButton>
      </div>
      <ElTable
        v-loading="searchingMessages"
      ...
      >
        ...
        <ElTableColumn label="发送者" min-width="130">
          <template #default="{ row }">
            <AdminEnumTag :value="row.senderMemberType" /> #{{
              row.senderMemberId
            }}
          </template>
        </ElTableColumn>
        <ElTableColumn prop="messageType" label="类型" width="95" />
        <ElTableColumn label="发送状态" width="110">
          <template #default="{ row }">
            <AdminEnumTag
              :value="row.status"
              :label="messageStatusText(row.status)"
            />
          </template>
        </ElTableColumn>
        <ElTableColumn label="审核状态" width="120">
          <template #default="{ row }">
            <AdminEnumTag
              :value="row.moderationStatus"
              :label="moderationStatusText(row.moderationStatus)"
            />
          </template>
        </ElTableColumn>
        ...
        <!-- @vue-generic {MessageRow} --><ElTableColumn
          label="操作"
          width="190"
          fixed="right"
        >
      ...
      <div class="admin-pagination">
        <ElButton
          :disabled="searchedCursorStack.length === 0"
    ...
    <ElDrawer
      v-model="detailOpen"
      :title="`会话 #${detail?.conversation.conversationId || ''}`"
      size="72%"
      @closed="detail = null"
    >
      <template v-if="detail">
        ...
        <div class="admin-pagination">
          <ElButton
            ...
            @click="loadMessages(messageNextCursor)"
          >
            加载更多
          </ElButton>
        </div>
    ...
    <ElDrawer
      ...
      title="消息上下文"
      size="55%"
      @closed="messageContext = null"
    >
      <template v-if="messageContext">
        <ElAlert
          title="仅展示同一会话的消息元数据，正文尚不可用。"
          type="warning"
          :closable="false"
          class="mb-4"
        />
        ...
        <ElDivider>前文</ElDivider><ElTable :data="messageContext.before" max-height="300">
          <ElTableColumn
            ...
            label="序号"
            width="80"
          /><ElTableColumn prop="messageId" label="消息 ID" /><ElTableColumn
            prop="messageType"
            label="类型"
            width="100"
          />
    ...
    <ElDrawer
      v-model="previewOpen"
      :title="`受控媒体预览 · ${previewMessageId}`"
      size="55%"
      @closed="clearPreview"
    >
      <ElAlert
        title="仅供受控查看，有效期 5 分钟；本页不提供下载。每次预览均单独记录审计。"
        type="warning"
        :closable="false"
        class="mb-4"
      />
      ...
      />
      <audio
        v-if="previewType === 'voice' && previewURL"
        :src="previewURL"
        controls
