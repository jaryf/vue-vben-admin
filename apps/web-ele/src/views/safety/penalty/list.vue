...
const createKey = ref('');
const filter = reactive({ userId: '', status: '', penaltyType: '', scope: '', referenceType: '', referenceId: '', createdBy: '', createdRange: [] as string[] });
...
const endsAtForbidden = computed(() => ['content_removed', 'permanent_ban'].includes(form.penaltyType));
const endsAtRequired = computed(() => form.penaltyType === 'temporary_suspension');
...
async function save() {
  ...
  if (form.userId < 1 || form.referenceId < 1 || !form.penaltyType || !code(form.scope) || !code(form.reasonCode) || !code(referenceType)) {
    ElMessage.error('请填写用户、处罚类型、范围、原因及关联记录'); return;
  }
  if (referenceType.length > 32) {
    ElMessage.error('关联类型不能超过 32 位'); return;
  }
  if (endsAtRequired.value && !form.endsAt.trim()) {
    ElMessage.error('临时封禁必须设置结束时间'); return;
  }
  if (endsAtForbidden.value && form.endsAt.trim()) {
    ElMessage.error('永久封禁和内容下架不能设置结束时间'); return;
  }
  let endsAt: null | string = null;
  try {
  ...
  if (endsAt && new Date(endsAt).getTime() <= Date.now()) {
    ElMessage.error('结束时间必须晚于当前时间'); return;
  }
  saving.value = true;
  try {
    await createPenalty({
...
<template>
  ...
  <ElDialog v-model="createOpen" title="创建处罚" width="540px"><ElForm label-width="110px"><ElFormItem label="用户 ID"><ElInputNumber v-model="form.userId" :min="1" /></ElFormItem><ElFormItem label="处罚类型"><ElSelect v-model="form.penaltyType" placeholder="请选择" @change="penaltyTypeChanged"><ElOption v-for="[value, label] in types" :key="value" :label="label" :value="value" /></ElSelect></ElFormItem><ElFormItem label="生效范围"><ElInput v-model="form.scope" maxlength="64" placeholder="稳定范围代码" /></ElFormItem><ElFormItem label="原因代码"><ElInput v-model="form.reasonCode" maxlength="64" show-word-limit /></ElFormItem><ElFormItem label="关联类型"><ElInput v-model="form.referenceType" maxlength="32" placeholder="如 report" /></ElFormItem><ElFormItem label="关联 ID"><ElInputNumber v-model="form.referenceId" :min="1" /></ElFormItem><ElFormItem label="结束时间"><ElDatePicker v-model="form.endsAt" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" :disabled="endsAtForbidden" :placeholder="endsAtRequired ? '临时封禁必须设置' : endsAtForbidden ? '当前类型不允许设置' : '留空表示无固定结束时间'" /><span class="ml-2 text-xs text-gray-500">{{ timezoneStore.timezone }}</span></ElFormItem></ElForm><template #footer><ElButton @click="createOpen = false">取消</ElButton><ElButton type="primary" :loading="saving" @click="save">创建</ElButton></template></ElDialog>
  </div>
</template>
