...
const createKey = ref('');
const filter = reactive({ userId: '', status: '', penaltyType: '', referenceType: '', referenceId: '' });
...
async function load(cursor = '') {
  ...
  try {
    const result = await listPenalties({
      userId: filter.userId || undefined, status: filter.status || undefined,
      penaltyType: filter.penaltyType || undefined, referenceType: filter.referenceType || undefined,
      referenceId: filter.referenceId || undefined, cursor: cursor || undefined, limit: 20,
    });
...
<template>
  <div class="p-5"><ElCard shadow="never">
    ...
    <div class="mb-4 flex flex-wrap gap-3"><ElInput v-model="filter.userId" placeholder="用户 ID" clearable class="!w-32" /><ElSelect v-model="filter.status" clearable placeholder="全部状态" class="!w-32"><ElOption label="生效中" value="active" /><ElOption label="已过期" value="expired" /><ElOption label="已撤销" value="revoked" /></ElSelect><ElSelect v-model="filter.penaltyType" clearable placeholder="处罚类型" class="!w-44"><ElOption v-for="[value, label] in types" :key="value" :label="label" :value="value" /></ElSelect><ElInput v-model="filter.referenceType" placeholder="关联类型" clearable class="!w-32" /><ElInput v-model="filter.referenceId" placeholder="关联 ID" clearable class="!w-32" /><ElButton @click="search">查询</ElButton></div>
    <ElTable v-loading="loading" :data="rows" row-key="penaltyId"><ElTableColumn prop="penaltyId" label="处罚 ID" width="100" /><ElTableColumn prop="userId" label="用户 ID" width="100" /><ElTableColumn label="类型" min-width="120"><template #default="{ row }">{{ typeText(row.penaltyType) }}</template></ElTableColumn><ElTableColumn prop="scope" label="范围" width="110" /><ElTableColumn prop="reasonCode" label="原因代码" min-width="145" /><ElTableColumn label="关联记录" min-width="150"><template #default="{ row }">{{ row.referenceType }} #{{ row.referenceId }}</template></ElTableColumn><ElTableColumn prop="status" label="状态" width="95" /><ElTableColumn prop="endsAt" label="结束时间" min-width="170" /><ElTableColumn label="操作" width="95"><template #default="{ row }"><ElButton v-if="canRevoke && row.status === 'active'" link type="warning" @click="revoke(row)">撤销</ElButton></template></ElTableColumn></ElTable>
