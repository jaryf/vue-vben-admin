  ...
  <div class="p-5">
    <ElCard shadow="never">
      ...
      <ElTable v-loading="loading" :data="rows" row-key="userId" class="w-full">
        ...
        <ElTableColumn prop="lastActiveAt" label="最近活跃" min-width="170"><template #default="{ row }"><AdminTime :value="row.lastActiveAt" /></template></ElTableColumn>
        <ElTableColumn prop="registeredAt" label="注册时间" min-width="170"><template #default="{ row }"><AdminTime :value="row.registeredAt" /></template></ElTableColumn>
        ...
        <ElTabs v-model="detailTab" @tab-change="detailTabChanged">
          ...
          <ElTabPane label="额度与权益" name="quota">
            <ElDescriptions :column="2" border>
              ...
              <ElDescriptionsItem label="重置时间"><AdminTime :value="detail.quota.resetAt" /></ElDescriptionsItem>
            ...
            <ElTable :data="detail.entitlements || []" class="mt-4"><ElTableColumn prop="entitlementType" label="权益类型" /><ElTableColumn prop="balance" label="余额" /><ElTableColumn prop="status" label="状态" /><ElTableColumn prop="expiresAt" label="到期时间"><template #default="{ row }"><AdminTime :value="row.expiresAt" /></template></ElTableColumn></ElTable>
          </ElTabPane>
          <ElTabPane label="会话与设备" name="sessions">
            <ElTable :data="sessions" row-key="sessionId"><ElTableColumn prop="sessionId" label="会话 ID" /><ElTableColumn prop="platform" label="平台" /><ElTableColumn prop="deviceModel" label="设备" /><ElTableColumn prop="lastUsedAt" label="最近使用"><template #default="{ row }"><AdminTime :value="row.lastUsedAt" /></template></ElTableColumn><ElTableColumn prop="status" label="状态" /><ElTableColumn v-if="canRevoke" label="操作"><template #default="{ row }"><ElButton v-if="row.status === 'active'" link type="danger" @click="revokeOne(row.sessionId)">撤销</ElButton></template></ElTableColumn></ElTable>
            <ElDivider>设备</ElDivider>
            <ElTable :data="devices" row-key="deviceId"><ElTableColumn prop="deviceId" label="设备 ID" /><ElTableColumn prop="platform" label="平台" /><ElTableColumn prop="deviceModel" label="型号" /><ElTableColumn prop="integrityStatus" label="完整性" /><ElTableColumn prop="lastActiveAt" label="最近活跃"><template #default="{ row }"><AdminTime :value="row.lastActiveAt" /></template></ElTableColumn></ElTable>
          </ElTabPane>
          <ElTabPane label="身份绑定" name="bindings"><ElTable :data="bindings"><ElTableColumn prop="provider" label="方式" /><ElTableColumn prop="displayValue" label="展示值" /><ElTableColumn prop="status" label="状态" /><ElTableColumn prop="boundAt" label="绑定时间"><template #default="{ row }"><AdminTime :value="row.boundAt" /></template></ElTableColumn></ElTable><ElDivider>绑定冲突</ElDivider><ElTable :data="conflicts"><ElTableColumn prop="conflictId" label="记录 ID" /><ElTableColumn prop="provider" label="方式" /><ElTableColumn prop="createdAt" label="时间"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable></ElTabPane>
          <ElTabPane v-if="canReadOrders" label="订单与账本" name="commerce">
            ...
            <ElTable v-if="commerceKind === 'orders'" v-loading="commerceLoading" :data="commerceRows" row-key="orderId"><ElTableColumn prop="orderId" label="订单 ID" width="100" /><ElTableColumn prop="orderNo" label="订单号" min-width="170" /><ElTableColumn prop="internalCode" label="商品" min-width="130" /><ElTableColumn prop="status" label="状态" width="120" /><ElTableColumn prop="amountMinor" label="最小货币单位金额" width="155" /><ElTableColumn prop="currency" label="币种" width="85" /><ElTableColumn prop="createdAt" label="创建时间" min-width="165"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn><ElTableColumn label="操作" width="80"><template #default="{ row }"><ElButton link type="primary" @click="openOrderDetail(row.orderId)">详情</ElButton></template></ElTableColumn></ElTable>
            <ElTable v-else-if="commerceKind === 'entitlements'" v-loading="commerceLoading" :data="commerceRows" row-key="ledgerId"><ElTableColumn prop="ledgerId" label="流水 ID" width="100" /><ElTableColumn prop="entitlementType" label="权益类型" min-width="150" /><ElTableColumn prop="operation" label="操作" width="105" /><ElTableColumn prop="changeAmount" label="变动" width="95" /><ElTableColumn prop="balanceAfter" label="变动后" width="95" /><ElTableColumn prop="sourceType" label="来源" width="110" /><ElTableColumn prop="createdAt" label="创建时间" min-width="165"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable>
            <ElTable v-else v-loading="commerceLoading" :data="commerceRows" row-key="ledgerId"><ElTableColumn prop="ledgerId" label="流水 ID" width="100" /><ElTableColumn prop="operation" label="操作" width="110" /><ElTableColumn prop="amount" label="变动" width="95" /><ElTableColumn prop="balanceAfter" label="变动后" width="95" /><ElTableColumn prop="sourceType" label="来源" width="120" /><ElTableColumn prop="orderId" label="关联订单" width="110" /><ElTableColumn prop="createdAt" label="创建时间" min-width="165"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable>
          ...
          <ElTabPane v-if="canReadActivity" label="内容与安全记录" name="activity">
            ...
            <ElTable v-if="activityKind === 'bottles'" v-loading="activityLoading" :data="activityRows" row-key="bottleId"><ElTableColumn prop="bottleId" label="漂流瓶 ID" width="120" /><ElTableColumn prop="contentType" label="类型" width="95" /><ElTableColumn prop="status" label="状态" width="120" /><ElTableColumn prop="reviewStatus" label="审核状态" width="120" /><ElTableColumn prop="reportCount" label="举报数" width="90" /><ElTableColumn prop="createdAt" label="创建时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable>
            <ElTable v-else-if="activityKind === 'conversations'" v-loading="activityLoading" :data="activityRows" row-key="conversationId"><ElTableColumn prop="conversationId" label="会话 ID" width="115" /><ElTableColumn prop="type" label="类型" width="110" /><ElTableColumn prop="status" label="状态" width="120" /><ElTableColumn prop="memberCount" label="成员数" width="90" /><ElTableColumn prop="messageCount" label="消息数" width="90" /><ElTableColumn prop="lastMessageAt" label="最近消息" min-width="175"><template #default="{ row }"><AdminTime :value="row.lastMessageAt" /></template></ElTableColumn></ElTable>
            <ElTable v-else-if="activityKind === 'reports'" v-loading="activityLoading" :data="activityRows" row-key="reportId"><ElTableColumn prop="reportId" label="举报 ID" width="105" /><ElTableColumn prop="targetType" label="目标类型" width="110" /><ElTableColumn prop="targetId" label="目标 ID" min-width="125" /><ElTableColumn prop="reasonCode" label="原因" min-width="125" /><ElTableColumn prop="priority" label="优先级" width="100" /><ElTableColumn prop="status" label="状态" width="110" /><ElTableColumn prop="createdAt" label="提交时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable>
            <ElTable v-else v-loading="activityLoading" :data="activityRows" row-key="eventId"><ElTableColumn prop="eventId" label="事件 ID" width="105" /><ElTableColumn prop="eventType" label="事件类型" min-width="155" /><ElTableColumn prop="severity" label="严重程度" width="105" /><ElTableColumn prop="sourceType" label="来源" width="115" /><ElTableColumn prop="reasonCode" label="原因" min-width="125" /><ElTableColumn prop="occurredAt" label="发生时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.occurredAt" /></template></ElTableColumn></ElTable>
          ...
          <ElTabPane label="状态记录" name="history"><ElTable :data="detail.recentStatusLogs"><ElTableColumn prop="fromStatus" label="原状态" /><ElTableColumn prop="toStatus" label="新状态" /><ElTableColumn prop="reasonCode" label="原因代码" /><ElTableColumn prop="createdAt" label="时间"><template #default="{ row }"><AdminTime :value="row.createdAt" /></template></ElTableColumn></ElTable></ElTabPane>
        </ElTabs>
      </template>
    </ElDrawer>
    ...
    <ElDialog v-model="deletionOpen" title="注销申请复核" width="85%" destroy-on-close>
      ...
      <ElTable :data="deletionRows" row-key="requestId">
        ...
        <ElTableColumn prop="requestedAt" label="申请时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.requestedAt" /></template></ElTableColumn>
        <ElTableColumn prop="scheduledFor" label="计划注销时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.scheduledFor" /></template></ElTableColumn>
