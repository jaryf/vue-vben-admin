...
const createKey = ref('');
const filter = reactive({ contentType: '', slug: '', languageCode: '', version: '', status: '' });
...
async function load(cursor = '') {
  loading.value = true;
  try {
    const result = await listWebsiteContent({ contentType: filter.contentType || undefined, slug: filter.slug.trim() || undefined, languageCode: filter.languageCode.trim() || undefined, version: filter.version.trim() || undefined, status: filter.status || undefined, cursor: cursor || undefined, limit: 20 });
    rows.value = result.items || []; nextCursor.value = result.nextCursor;
  } finally { loading.value = false; }
}
function search() {
  if (filter.slug.trim() && !/^[a-z0-9][a-z0-9-]{0,79}$/.test(filter.slug.trim())) { ElMessage.warning('路径标识只能包含小写字母、数字和连字符'); return; }
  if (filter.languageCode.trim() && !/^[a-zA-Z]{2,3}(-[a-zA-Z]{2})?$/.test(filter.languageCode.trim())) { ElMessage.warning('请输入有效的语言代码'); return; }
  if (filter.version.trim() && !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(filter.version.trim())) { ElMessage.warning('请输入有效的版本号'); return; }
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>官网内容与帮助文章</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    ...
    <div class="mb-4 flex flex-wrap gap-3"><ElSelect v-model="filter.contentType" clearable placeholder="全部内容类型" class="!w-40"><ElOption label="官网页面" value="page" /><ElOption label="帮助文章" value="help_article" /></ElSelect><ElInput v-model="filter.slug" placeholder="路径标识" clearable class="!w-40" /><ElInput v-model="filter.languageCode" placeholder="语言代码" clearable class="!w-32" /><ElInput v-model="filter.version" placeholder="版本号" clearable class="!w-36" /><ElSelect v-model="filter.status" clearable placeholder="全部状态" class="!w-32"><ElOption label="草稿" value="draft" /><ElOption label="已发布" value="published" /><ElOption label="已退役" value="retired" /></ElSelect><ElButton @click="search">查询</ElButton></div>
    <ElTable v-loading="loading" :data="rows" row-key="versionId"><ElTableColumn prop="versionId" label="版本 ID" width="100" /><ElTableColumn label="类型" width="105"><template #default="{ row }">{{ typeText(row.contentType) }}</template></ElTableColumn><ElTableColumn prop="slug" label="路径标识" min-width="145" /><ElTableColumn prop="languageCode" label="语言" width="85" /><ElTableColumn prop="version" label="版本" width="110" /><ElTableColumn prop="title" label="标题" min-width="185" /><ElTableColumn label="状态" width="90"><template #default="{ row }">{{ statusText(row.status) }}</template></ElTableColumn><ElTableColumn prop="reviewedBy" label="复核人" width="90" /><ElTableColumn prop="updatedAt" label="更新时间" min-width="175"><template #default="{ row }"><AdminTime :value="row.updatedAt" /></template></ElTableColumn><ElTableColumn label="操作" min-width="235" fixed="right"><template #default="{ row }"><ElButton link type="primary" @click="openDetail(row)">详情</ElButton><ElButton v-if="canWrite && row.status === 'draft'" link type="primary" @click="openEditor(row)">编辑</ElButton><ElButton v-if="canReview && row.status === 'draft'" link type="success" @click="action(row, 'review')">复核</ElButton><ElButton v-if="canPublish && row.status === 'draft' && row.reviewedBy" link type="success" @click="action(row, 'publish')">发布</ElButton><ElButton v-if="canPublish && row.status === 'published'" link type="warning" @click="action(row, 'retire')">退役</ElButton></template></ElTableColumn></ElTable><div class="mt-4 flex justify-end gap-2"><ElButton :disabled="cursorStack.length === 0" @click="previous">上一页</ElButton><ElButton :disabled="!nextCursor" @click="next">下一页</ElButton></div>
  </ElCard><ElDrawer v-model="detailOpen" title="内容版本详情" size="65%" @closed="detail = null"><ElDescriptions v-if="detail" :column="2" border><ElDescriptionsItem label="路径">{{ detail.slug }}</ElDescriptionsItem><ElDescriptionsItem label="语言">{{ detail.languageCode }}</ElDescriptionsItem><ElDescriptionsItem label="类型">{{ typeText(detail.contentType) }}</ElDescriptionsItem><ElDescriptionsItem label="版本">{{ detail.version }}</ElDescriptionsItem><ElDescriptionsItem label="标题">{{ detail.title }}</ElDescriptionsItem><ElDescriptionsItem label="状态">{{ statusText(detail.status) }}</ElDescriptionsItem></ElDescriptions><ElDivider>Markdown 安全预览</ElDivider><ElAlert v-if="detail" class="mb-4" title="原始 HTML 已禁用，HTML 标签只会作为文本显示。" type="info" show-icon :closable="false" /><SafeMarkdownPreview v-if="detail" :source="detail.bodyMarkdown" /></ElDrawer>
  <ElDrawer v-model="editorOpen" :title="editingId === null ? '创建内容草稿' : `编辑草稿 #${editingId}`" size="65%"><ElForm label-width="100px"><ElFormItem label="内容类型"><ElSelect v-model="form.contentType" :disabled="editingId !== null"><ElOption label="官网页面" value="page" /><ElOption label="帮助文章" value="help_article" /></ElSelect></ElFormItem><ElFormItem label="路径标识"><ElInput v-model="form.slug" :disabled="editingId !== null" placeholder="小写字母、数字和连字符" /></ElFormItem><ElFormItem label="语言代码"><ElInput v-model="form.languageCode" :disabled="editingId !== null" /></ElFormItem><ElFormItem label="版本"><ElInput v-model="form.version" /></ElFormItem><ElFormItem label="标题"><ElInput v-model="form.title" /></ElFormItem><ElFormItem label="Markdown"><ElInput v-model="form.bodyMarkdown" type="textarea" :rows="18" /></ElFormItem><ElDivider>安全预览</ElDivider><SafeMarkdownPreview :source="form.bodyMarkdown" /></ElForm><template #footer><ElButton @click="editorOpen = false">取消</ElButton><ElButton type="primary" :loading="saving" @click="save">保存草稿</ElButton></template></ElDrawer></div>
</template>
