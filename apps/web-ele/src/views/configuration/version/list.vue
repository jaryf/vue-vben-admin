...
const status = ref('');
const scope = ref('');
const form = reactive({ scope: 'feature_flags' as ConfigVersion['scope'], version: '', blockedTerms: '', regexRules: '', maxPercent: 0, rolloutPercent: 0, languageOverrides: '', states: {} as Record<FlagName, 'false' | 'inherit' | 'true'> });
...
async function save() {
  ...
  if (form.scope === 'feature_flags') {
    ...
    if (terms.length + patterns.length === 0 || terms.length > 100 || patterns.length > 20 || terms.some((value) => [...value].length < 2 || [...value].length > 64 || /[\x00-\x1F]/.test(value)) || new Set(terms.map((value) => value.toLocaleLowerCase())).size !== terms.length) { ElMessage.error('请填写拦截词或正则规则；拦截词最多 100 条、每条 2 至 64 字且不能重复'); return; }
    ...
    payload = { ...(terms.length ? { blockedTerms: terms } : {}), ...(patterns.length ? { regexRules: patterns } : {}) };
  } else {
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>版本化配置与审核规则</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    <ElAlert class="mb-4" title="功能开关不能开启基础配置禁用的能力。附加审核词与正则在同步消息和异步文字瓶审核中生效，固定联系方式和外链规则始终有效。AI 投放默认 0%，单语种最高 20%；发布与回滚均需异人审批。" type="info" show-icon :closable="false" />
