...
const status = ref('');
const scope = ref('');
const form = reactive({ scope: 'feature_flags' as ConfigVersion['scope'], version: '', blockedTerms: '', states: {} as Record<FlagName, 'false' | 'inherit' | 'true'> });
...
const scopeText = (value: string) => value === 'feature_flags' ? '功能开关' : '附加审核规则';
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>版本化配置与审核规则</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    <ElAlert class="mb-4" title="功能开关只覆盖 App 启动接口且不能开启服务端基础配置禁用的能力。附加审核词在同步消息和异步漂流瓶审核中生效；固定的联系方式与外链规则始终保留。发布与回滚均需申请人之外的管理员批准。" type="info" show-icon :closable="false" />
    <div class="mb-4 flex gap-3"><ElSelect v-model="scope" clearable placeholder="全部类型" class="!w-40"><ElOption label="功能开关" value="feature_flags" /><ElOption label="附加审核规则" value="moderation_rules" /></ElSelect><ElSelect v-model="status" clearable placeholder="全部状态" class="!w-36"><ElOption label="草稿" value="draft" /><ElOption label="待审批" value="reviewing" /><ElOption label="已批准" value="approved" /><ElOption label="已发布" value="published" /><ElOption label="已退役" value="retired" /></ElSelect><ElButton @click="search">查询</ElButton></div>
  ...
  </ElCard><ElDrawer v-model="detailOpen" title="配置版本详情" size="55%" @closed="detail = null"><ElDescriptions v-if="detail" :column="2" border><ElDescriptionsItem label="类型">{{ scopeText(detail.scope) }}</ElDescriptionsItem><ElDescriptionsItem label="版本">{{ detail.version }}</ElDescriptionsItem><ElDescriptionsItem label="状态">{{ statusText(detail.status) }}</ElDescriptionsItem><ElDescriptionsItem label="申请人">{{ detail.requestedBy }}</ElDescriptionsItem><ElDescriptionsItem label="审批人">{{ detail.reviewedBy ?? '—' }}</ElDescriptionsItem><ElDescriptionsItem label="审批说明">{{ detail.reviewNote || '—' }}</ElDescriptionsItem><ElDescriptionsItem label="回滚来源">{{ detail.rollbackOf ?? '—' }}</ElDescriptionsItem></ElDescriptions><ElDivider>{{ detail?.scope === 'feature_flags' ? '能力配置' : '附加拦截词' }}</ElDivider><ElDescriptions v-if="detail?.scope === 'feature_flags'" :column="1" border><ElDescriptionsItem v-for="[name, label] in flags" :key="name" :label="label">{{ configuredFlags(detail)[name] === undefined ? '继承基础配置' : configuredFlags(detail)[name] ? '开启' : '关闭' }}</ElDescriptionsItem></ElDescriptions><div v-else-if="detail" class="flex flex-wrap gap-2"><ElTag v-for="term in configuredTerms(detail)" :key="term">{{ term }}</ElTag></div></ElDrawer>
  <ElDrawer v-model="editorOpen" :title="editingId === null ? '创建配置草稿' : `编辑草稿 #${editingId}`" size="55%"><ElForm label-width="150px"><ElFormItem label="配置类型"><ElSelect v-model="form.scope" :disabled="editingId !== null"><ElOption label="功能开关" value="feature_flags" /><ElOption label="附加审核规则" value="moderation_rules" /></ElSelect></ElFormItem><ElFormItem label="版本号"><ElInput v-model="form.version" placeholder="例如 2026.09.24-1" /></ElFormItem><template v-if="form.scope === 'feature_flags'"><ElDivider>能力覆盖</ElDivider><ElFormItem v-for="[name, label] in flags" :key="name" :label="label"><ElSelect v-model="form.states[name]"><ElOption label="继承基础配置" value="inherit" /><ElOption label="开启（需基础配置支持）" value="true" /><ElOption label="关闭" value="false" /></ElSelect></ElFormItem></template><ElFormItem v-else label="拦截词"><ElInput v-model="form.blockedTerms" type="textarea" :rows="14" placeholder="每行一个拦截词；不会替换固定安全规则" /></ElFormItem></ElForm><template #footer><ElButton @click="editorOpen = false">取消</ElButton><ElButton type="primary" :loading="saving" @click="save">保存草稿</ElButton></template></ElDrawer></div>
</template>
