...
const published = ref<ConfigVersion | null>(null);
const editingId = ref<null | number>(null);
const createKey = ref('');
const status = ref('');
const scope = ref('');
const form = reactive({ scope: 'feature_flags' as ConfigVersion['scope'], version: '', blockedTerms: '', regexRules: '', blockedDomains: '', shortlinkDomains: '', riskMedium: 0.4, riskHigh: 0.7, riskCritical: 0.9, manualReviewBelow: 0.7, maxPercent: 0, rolloutPercent: 0, languageOverrides: '', states: {} as Record<FlagName, 'false' | 'inherit' | 'true'> });
...
const scopeText = (value: string) => ({ feature_flags: '功能开关', moderation_rules: '审核规则与风险阈值', ai_distribution: 'AI 投放比例' })[value as ConfigVersion['scope']] || value;
...
function payloadFields(row: ConfigVersion): Record<string, string> {
  ...
  if (row.scope === 'moderation_rules') {
    ...
    return {
      附加拦截词: configuredTerms(row).join('、') || '无',
      附加正则: configuredPatterns(row).join('、') || '无',
      附加拦截域名: configuredDomains(row, 'blockedDomains').join('、') || '无',
      短链域名: configuredDomains(row, 'shortlinkDomains').join('、') || '无',
      中风险起点: String(risk?.medium ?? 0.4),
      高风险起点: String(risk?.high ?? 0.7),
      极高风险起点: String(risk?.critical ?? 0.9),
      人工复核边界: String(risk?.manualReviewBelow ?? 0.7),
    };
  }
  const ai = configuredAI(row);
...
async function openDetail(row: ConfigVersion) {
  const [current, page] = await Promise.all([
    ...
    listConfigVersions({ scope: row.scope, status: 'published', limit: 1 }),
  ]);
  detail.value = current;
  published.value = page.items?.[0] || null;
...
async function openEditor(row?: ConfigVersion) {
  ...
  form.riskCritical = risk?.critical ?? 0.9;
  form.manualReviewBelow = risk?.manualReviewBelow ?? 0.7;
...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>版本化配置与审核规则</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    <ElAlert class="mb-4" title="功能开关不能开启基础配置禁用的能力。附加审核词、正则、域名与短链规则在同步消息和异步文字瓶审核中生效，风险阈值用于异步文字瓶供应商评分；固定联系方式和外链规则始终有效。AI 投放默认 0%，单语种最高 20%；发布与回滚均需异人审批。" type="info" show-icon :closable="false" />
    <div class="mb-4 flex gap-3"><ElSelect v-model="scope" clearable placeholder="全部类型" class="!w-40"><ElOption label="功能开关" value="feature_flags" /><ElOption label="审核规则与风险阈值" value="moderation_rules" /><ElOption label="AI 投放比例" value="ai_distribution" /></ElSelect><ElSelect v-model="status" clearable placeholder="全部状态" class="!w-36"><ElOption label="草稿" value="draft" /><ElOption label="待审批" value="reviewing" /><ElOption label="已批准" value="approved" /><ElOption label="已发布" value="published" /><ElOption label="已退役" value="retired" /></ElSelect><ElButton @click="search">查询</ElButton></div>
