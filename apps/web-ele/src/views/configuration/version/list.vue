...
<template>
  <div class="p-5"><ElCard shadow="never"><template #header><div class="flex items-center justify-between"><span>版本化配置与审核规则</span><ElButton v-if="canWrite" type="primary" @click="openEditor()">创建草稿</ElButton></div></template>
    <ElAlert class="mb-4" title="功能开关不能开启基础配置禁用的能力。附加审核词、正则、域名与短链规则在同步消息和异步文字瓶审核中生效，风险阈值用于异步文字瓶供应商评分；固定联系方式和外链规则始终有效。AI 投放默认 0%，单语种最高 20%；目标兴趣、每日时段和单瓶曝光上限也通过版本审批生效。发布与回滚均需异人审批。" type="info" show-icon :closable="false" />
