...
const canResolve = computed(() =>
  hasAccessByCodes(['contact_request.resolve']),
);
...
async function load(cursor = '') {
  ...
  try {
    ...
    const result = await listContactRequests({
      contactRequestId: filter.id.trim() || undefined,
      status: filter.status || undefined,
      senderEmail: filter.senderEmail.trim() || undefined,
      assignedAdminId: filter.assignedAdminId || undefined,
      requestId: filter.requestId.trim() || undefined,
      language: filter.language.trim().toLowerCase() || undefined,
...
function search() {
  if (
    [filter.id, filter.assignedAdminId].some(
      (value) => value.trim() && !/^[1-9]\d*$/.test(value.trim()),
    )
  ) {
    ElMessage.warning('请求 ID 和管理员 ID 必须为正整数');
    return;
  }
  try {
...
async function openDetail(row: ContactRow) {
  const result = await promptDialog(
    `查看联系请求 #${row.contactRequestId} 全文，请填写审计原因`,
    '审计原因',
  ...
  detail.value = await getContactRequest(
    row.contactRequestId,
    detailReason.value,
  );
...
async function refreshDetail() {
  if (detail.value)
    detail.value = await getContactRequest(
      detail.value.contactRequestId,
      detailReason.value,
    );
}
async function assign() {
  if (!detail.value) return;
  const result = await promptDialog('请输入接手管理员 ID', '分派联系请求', {
    inputValue: String(detail.value.assignedAdminId || ''),
    inputPattern: /^[1-9]\d*$/,
    inputErrorMessage: '请输入有效管理员 ID',
  });
  if (!result) return;
  const { value } = result;
...
function openReply() {
  ...
  Object.assign(replyForm, {
    subject: `Re: ${detail.value.subject}`,
    message: '',
  });
  replyKey.value = crypto.randomUUID();
  replyOpen.value = true;
}
async function reply() {
  if (!detail.value || !replyForm.subject.trim() || !replyForm.message.trim()) {
    ElMessage.error('请填写主题与回复正文');
    return;
  }
  sending.value = true;
  try {
    detail.value = await replyContactRequest(
      detail.value.contactRequestId,
    ...
    ElMessage[latest?.deliveryStatus === 'sent' ? 'success' : 'warning'](
      latest?.deliveryStatus === 'sent'
        ? '回复已发送'
        : `回复未送达：${latest?.deliveryErrorCode || '外部邮件服务不可用'}`,
    );
    await load(cursorStack.value.at(-1) || '');
  } finally {
    sending.value = false;
...
async function resolve() {
  if (!detail.value) return;
  const result = await promptDialog('请输入处理结论', '解决联系请求', {
    inputValidator: (text) => !!text.trim() || '请填写处理结论',
  });
  if (!result) return;
  const { value } = result;
  detail.value = await resolveContactRequest(
    detail.value.contactRequestId,
    value.trim(),
  );
  ElMessage.success('联系请求已解决');
  await load(cursorStack.value.at(-1) || '');
}
onMounted(() => {
  void load();
...
<template>
  <AdminPage
    title="联系请求"
    description="跟进官网来信、分派处理与回复投递状态。"
  >
    <ElCard shadow="never">
      <ElAlert
        title="邮件通知与回复依赖外部邮件服务。发送失败时保留真实投递状态和错误码，不视为已送达。全文查看需填写审计原因。"
        type="info"
        show-icon
        :closable="false"
        class="mb-4"
      ...
      <ElTable v-loading="loading" :data="rows" row-key="contactRequestId">
        <ElTableColumn
          prop="contactRequestId"
          label="请求 ID"
          width="100"
        /><ElTableColumn
          prop="senderName"
        ...
        /><!-- @vue-generic {ContactRow} --><ElTableColumn
          label="状态"
          width="120"
        >
    ...
    >
      <template v-if="detail">
        ...
        <div class="mt-4 flex gap-2">
          <ElButton
            v-if="canAssign && detail.status !== 'resolved'"
            @click="assign"
          >
            分派
</ElButton><ElButton
        ...
        <ElDivider>回复记录</ElDivider><ElTable :data="detail.replies">
          ...
          /><ElTableColumn
            ...
            min-width="220"
            show-overflow-tooltip
          /><ElTableColumn prop="deliveryStatus" label="投递状态" width="105">
            <template #default="{ row }">
            ...
            label="错误码"
            min-width="145"
          /><ElTableColumn prop="sentAt" label="发送时间" min-width="165">
            <template #default="{ row }">
              <AdminTime :value="row.sentAt" />
            </template>
          </ElTableColumn>
        </ElTable>
      </template>
    </ElDrawer>
    <ElDialog v-model="replyOpen" title="回复联系请求" width="650px">
