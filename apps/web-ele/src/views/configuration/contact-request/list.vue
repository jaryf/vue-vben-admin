...
const filter = reactive({ id: '', status: '', senderEmail: '', assignedAdminId: '', requestId: '', language: '', createdRange: [] as string[] });
...
async function openDetail(row: ContactRow) {
  const { value } = await ElMessageBox.prompt(`查看联系请求 #${row.contactRequestId} 全文，请填写审计原因`, '审计原因', { inputValidator: validateReasonCode });
