...
async function save() {
  if (
    !form.username.trim() ||
    (editingId.value === null && form.password.length < 8)
  ) {
  ...
  try {
    ...
    await (editingId.value === null
      ...
      : updateAdminUser(editingId.value, data));
...
async function submitReset() {
  if (
    ...
    !/^\d{6}$/.test(resetForm.totpCode) ||
    resetForm.reason.trim().length < 8
  ) {
  ...
  saving.value = true;
  try {
    await resetAdminMFA(resetTarget.value.id, {
      ...resetForm,
...
<template>
  ...
  >
    ...
    <section class="admin-panel">
      <div class="mb-5">
        <h2 class="text-base font-semibold text-foreground">管理员列表</h2>
      ...
      <div class="admin-filter">
        ...
        <ElSelect
          ...
          class="!w-36"
        >
          <ElOption label="正常" value="1" /><ElOption label="停用" value="2" />
        </ElSelect>
      ...
      <ElTable v-loading="loading" :data="rows" row-key="id" class="w-full">
        ...
        <ElTableColumn label="双因素认证" width="125">
          <template #default="{ row }">
            <AdminEnumTag
              :value="row.mfaEnabled"
              :label="row.mfaEnabled ? '已启用' : '未启用'"
              :tone="row.mfaEnabled ? 'success' : 'warning'"
            />
          </template>
        </ElTableColumn>
        ...
        <!-- @vue-generic {AdminUser} --><ElTableColumn
        ...
        >
          <template #default="{ row }">
            ...
            <ElButton
              v-if="canUpdate && row.id !== Number(userStore.userInfo?.userId)"
              link
              type="warning"
              @click="openReset(row)"
            >
            ...
            <ElButton
              v-if="canDelete && row.id !== Number(userStore.userInfo?.userId)"
              link
              type="danger"
              @click="remove(row)"
            >
    ...
    >
      ...
      >
        ...
        <ElFormItem v-if="editingId === null" label="初始密码">
          <ElInput
        ...
        <ElFormItem label="角色">
          <ElSelect v-model="form.roleIds" multiple class="w-full">
            <ElOption
              v-for="role in roles"
              :key="role.id"
              :label="role.name"
    ...
    <ElDialog
      v-model="resetOpen"
      :title="`重置 ${resetTarget?.username || ''} 的双因素认证`"
      width="520px"
      destroy-on-close
    >
      <ElAlert
        title="此操作会撤销目标管理员的当前会话，并记录审计。"
        type="warning"
        show-icon
        :closable="false"
        class="mb-5"
      ...
      <ElForm label-position="top" @submit.prevent="submitReset">
        ...
        <ElFormItem label="你的动态验证码">
          <ElInput
            v-model="resetForm.totpCode"
            maxlength="6"
            inputmode="numeric"
          />
        </ElFormItem>
      ...
      <template #footer>
        ...
        <ElButton type="danger" :loading="saving" @click="submitReset">
          确认重置
        </ElButton>
      </template>
    </ElDialog>
