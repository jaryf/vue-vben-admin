...
const treeRef = ref<null | {
  ...
  setCheckedKeys: (ids: number[]) => void;
}>(null);
...
async function save() {
  ...
  try {
    ...
    await (editingId.value === null
      ? createAdminRole({ ...data, code: form.code.trim() })
      : updateAdminRole(editingId.value, data));
...
async function remove(row: AdminRole) {
  if (
    !(await confirmDialog(`确定删除角色「${row.name}」吗？`, '删除角色', {
      type: 'warning',
    }))
  )
    return;
...
<template>
  <AdminPage
    title="角色管理"
    description="按职责分配菜单和操作权限，管理团队的访问范围。"
  >
    <template #actions>
    ...
    <section class="admin-panel">
      <div class="mb-5">
        <h2 class="text-base font-semibold text-foreground">角色列表</h2>
      ...
      <ElTable v-loading="loading" :data="rows" row-key="id" class="w-full">
        ...
        <!-- @vue-generic {AdminRole} --><ElTableColumn
          ...
          <div class="max-h-80 w-full overflow-auto rounded border p-3">
            ...
            >
              <template #default="{ data }">
                <span>
                  <span>{{ data.name }}</span>
                  <span v-if="data.authCode" class="ml-2 text-xs text-gray-400">
                    {{ data.authCode }}
                  </span>
                </span>
              </template>
